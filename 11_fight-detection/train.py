...
def train_model(num_epochs, num_classes, lr, device, save_dir, train_dataloader, val_dataloader, test_dataloader):
    ...
    # 保存训练的好权重 .tar文件保存在save_dir，而日志文件保存在save_dir/models
...
if __name__ == "__main__":
    ...
    train_data = VideoDataset(dataset_path='./data/Fight_data', images_path='train', clip_len=16)
    train_dataloader = DataLoader(train_data, batch_size=32, shuffle=True, num_workers=1)
    val_data = VideoDataset(dataset_path='./data/Fight_data', images_path='val', clip_len=16)
    val_dataloader = DataLoader(val_data, batch_size=32, num_workers=1)
    test_data = VideoDataset(dataset_path='./data/Fight_data', images_path='test', clip_len=16)
    test_dataloader = DataLoader(test_data, batch_size=32, num_workers=1)
